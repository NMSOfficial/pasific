  ...
  const loadItems = async (batchId: string) => {
    ...
    setMatches((current) => {
      ...
      for (const item of rows) {
        ...
        if (candidates.length === 1) next[item.id] = candidates[0].id;
      }
      return next;
    });
  };
  ...
  const importFiles = async (files: File[], mode: DocumentSourceMode) => {
    ...
    for (const file of files) {
      ...
      try {
        ...
        // The row already records its OCR error; continue the remaining documents.
      }
  ...
  const gradeItem = async (item: DocumentImportItem) => {
    ...
    try {
      ...
      setItems((current) => current.map((row) => row.id === item.id
        ? { ...row, studentId, linkedSubmissionId: submissionId, reviewStatus: 'teacher_review_pending' }
        : row));
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      <section className="card card--padded assessment-form-section">
        ...
        {assignments.length === 0 ? <p className="field__hint">Önce yayınlanmış bir writing ödevi oluştur.</p> : (
      ...
      {items.length > 0 && (
        <section className="card card--padded assessment-form-section">
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 'var(--space-3)', alignItems: 'center' }}>
            <div><h2>3. Öğrenciyi doğrula ve AI'ya gönder</h2><p className="field__hint">OCR isim önerisi yalnızca yardımcıdır; son eşleştirme öğretmene aittir.</p></div>
            {activeBatchId && <button type="button" className="icon-btn" onClick={() => loadItems(activeBatchId)} aria-label="Yenile"><RefreshCw size={17} /></button>}
          </div>
          <div className="ocr-item-list">
            {items.map((item) => (
              <article key={item.id} className="ocr-item-card">
                <div className="ocr-item-card__header">
                  <div><strong>{item.originalFilename}</strong><div className="field__hint">OCR: {item.ocrStatus} · {item.pageCount ?? 0} sayfa {item.confidence !== undefined ? `· güven ${Math.round(item.confidence * 100)}%` : ''}</div></div>
                ...
                <div className="ocr-item-card__actions">
                  <select className="select-control" value={matches[item.id] ?? ''} onChange={(event) => setMatches((current) => ({ ...current, [item.id]: event.target.value }))} disabled={item.ocrStatus !== 'ready'}>
                    <option value="">Öğrenci seç…</option>
                    {eligibleStudents.map((student) => <option key={student.id} value={student.id}>{student.displayName} · @{student.username}</option>)}
                  </select>
                  {item.linkedSubmissionId ? (
                    <Link className="btn btn--secondary" to={`/teacher/submissions/${item.linkedSubmissionId}`}>Puanı incele</Link>
                  ) : (
                    <button type="button" className="btn btn--primary" disabled={item.ocrStatus !== 'ready' || !matches[item.id] || busyItem === item.id} onClick={() => gradeItem(item)}>
                      <FileSearch size={16} /> AI puanla
                    </button>
                  )}
                </div>
              </article>
...
function ExamWorkspace({ schoolId, teacher, exams, allStudents, onExamCreated }: {
  ...
  const loadItems = async (batchId: string) => {
    ...
    setMatches((current) => {
      ...
      for (const item of rows) {
        if (next[item.id]) continue;
        const suggestion = normalized(item.suggestedStudentName || item.suggestedStudentIdentifier);
        const candidates = suggestion ? allStudents.filter((student) => [normalized(student.displayName), normalized(student.username)].some((value) => value === suggestion || value.includes(suggestion))) : [];
        if (candidates.length === 1) next[item.id] = candidates[0].id;
      }
      return next;
    });
  ...
  const importForExam = async (files: File[], mode: DocumentSourceMode, kind: 'exam_template' | 'exam_attempt') => {
    ...
    for (const file of files) {
      const item = await createDocumentItem({ batchId: batch.id, filename: file.name, mimeType: file.type });
      try { await processDocumentItem({ itemId: item.id, dataBase64: await fileToBase64(file), mimeType: file.type }); } catch { /* row records error */ }
      await loadItems(batch.id);
  ...
  const createAttemptAndGrade = async (item: DocumentImportItem) => {
    ...
    try {
      ...
      setItems((current) => current.map((row) => row.id === item.id ? { ...row, studentId, linkedExamAttemptId: attempt.id, reviewStatus: 'teacher_review_pending' } : row));
    } catch (error) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      <section className="assessment-two-column">
        ...
        <div className="card card--padded assessment-form-section">
          <h2>Sınav seç</h2>
          <select className="select-control" value={examId} onChange={(event) => { setExamId(event.target.value); setItems([]); setMatches({}); }}>
      ...
      {items.length > 0 && exam?.masterOcrText && (
        <section className="card card--padded assessment-form-section">
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><div><h2>3. Öğrenci eşleştir ve puanlat</h2><p className="field__hint">AI sonucu doğrudan yayınlanmaz; her sınav “Onay bekliyor” durumunda öğretmene gelir.</p></div>{activeBatchId && <button className="icon-btn" onClick={() => loadItems(activeBatchId)}><RefreshCw size={17} /></button>}</div>
          <div className="ocr-item-list">
            {items.map((item) => (
              <article key={item.id} className="ocr-item-card">
                <div className="ocr-item-card__header"><div><strong>{item.originalFilename}</strong><div className="field__hint">OCR: {item.ocrStatus}</div></div><StatusBadge status={item.reviewStatus} /></div>
                {item.suggestedStudentName && <p className="field__hint">OCR isim önerisi: <strong>{item.suggestedStudentName}</strong></p>}
                {item.ocrText && <details><summary>OCR metni</summary><pre className="ocr-text-preview">{item.ocrText}</pre></details>}
                <div className="ocr-item-card__actions">
                  <select className="select-control" value={matches[item.id] ?? ''} onChange={(event) => setMatches((current) => ({ ...current, [item.id]: event.target.value }))} disabled={item.ocrStatus !== 'ready'}>
                    <option value="">Öğrenci seç…</option>
                    {allStudents.map((student) => <option key={student.id} value={student.id}>{student.displayName} · @{student.username}</option>)}
                  </select>
                  {item.linkedExamAttemptId ? <Link className="btn btn--secondary" to={`/teacher/exams/${exam.id}/attempts/${item.linkedExamAttemptId}`}>Puanı incele</Link> : <button type="button" className="btn btn--primary" disabled={item.ocrStatus !== 'ready' || !matches[item.id] || busyItem === item.id} onClick={() => createAttemptAndGrade(item)}><FileSearch size={16} /> AI puanla</button>}
                </div>
              </article>
            ))}
          </div>
...
function StatusBadge({ status }: { status: DocumentImportItem['reviewStatus'] }) {
  const label = status === 'teacher_review_pending' ? 'Onay bekliyor'
    : status === 'approved' ? 'Onaylandı'
      : status === 'returned' ? 'Geri gönderildi'
        : status === 'ready_for_grading' ? 'Puanlamaya hazır'
          : 'Eşleştirme bekliyor';
